...
public class publishersearch extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			while(r.next())
			{
				if(r.getString(6).equals(publisher))
				{
					c=1;
					k++;
			...
			if(c==0)
			{
				p.println("<marquee behavior=alternate scrollamount=12><h1><i>Required publisher books are not there in library</i></h1></marquee>");

			}
			else
			{
				p.println("<marquee behavior=alternate scrollamount=12><h1><i>"+k+" Required publisher books are avalible in library</i></h1></marquee>");
